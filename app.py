...
if uploaded_file is not None:
    try:
        with st.spinner("Procesando registros aduaneros..."):
            ...
            # 6. Deduplicación (incluye Tipo Ingreso)
            columnas_dedup = [
                col for col in [
...
if st.session_state.file_processed and st.session_state.df_final is not None:
    ...
    if 'Tipo Ingreso' in df_res.columns:
        ...
        tipo_sel = st.selectbox("Filtrar por Tipo de Ingreso:", tipos_disponibles, index=0)
        if tipo_sel != "TODOS":
    ...
    with col_exp2:
        def convert_df_to_excel(df_styled):
            ...
            output = BytesIO()
            # Convertir Styler a DataFrame plano para exportar
            try:
                df_export = df_styled.data
            except AttributeError:
                df_export = df_styled
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                df_export.to_excel(writer, index=False, sheet_name='Control_Bloqueos')
